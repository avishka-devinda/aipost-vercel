...
const totalLength = truncatedBody.length + toolResult.headline.length + toolResult.hashtags.length + 10; // 10 for newlines and spaces

if (totalLength > maxCharLimit) {
  // Calculate how many characters to keep from the body
  const charsToKeep = maxCharLimit - (toolResult.headline.length + toolResult.hashtags.length + 10);
...
const escapedBody = escapeMarkdownV2(truncatedBody);
      ...
      await bot.api.sendPhoto(TELEGRAM_CHANNEL_ID, ogurl, {
        caption: message,
        parse_mode: "MarkdownV2",
      });

...
export async function fetchLatestTechNews() {
  ...
  const backgroundProcessing = new Promise(async (resolve) => {
    try {
      ...
      console.log(`Processed ${feed.items.length} items, found ${nonPoliticalItems.length} non-political items`);
    ...
    const nonPoliticalItems = feed.items.filter(item => {
      ...
      return !item.categories.some((category:FeedCategory) => {
        ...
        if (typeof category === 'string') {
          return category === 'Poliics' || category === 'Polcy';
        }
        
        // Handle object with term property
        if (category && typeof category === 'object') {
          if (category.term) {
            return category.term === 'Potics' || category.term === 'Polcy';
          }
          
          if (category.$ && category.$.term) {
            return category.$.term === 'Polics' || category.$.term === 'Poicy';
          }
        }
